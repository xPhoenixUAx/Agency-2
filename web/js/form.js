...
if(form){
  ...
  retry.type='button';retry.className='button secondary retry-session';retry.textContent='Retry connection';retry.hidden=true;
  status.after(retry);
  ...
  async function connect(){
    retry.disabled=true;
    try{
      const [c]=await Promise.all([configReady,token()]);cfg=c;submit.disabled=false;retry.hidden=true;
      if(new URLSearchParams(location.search).get('need')==='tracking' && c.form.needs.includes('Tracking & Analytics')) form.elements.need.value='Tracking & Analytics';
      say('');
    }catch{say('The form is unavailable. Retry the connection or use the contact email on this page.','error');retry.hidden=false;}
  ...
  form.addEventListener('submit',async event=>{
    ...
    sending=true;submit.disabled=true;submit.textContent='Sending…';form.setAttribute('aria-busy','true');say('Sending your request…');
    ...
    try{
      ...
      if(!response.ok || !result.ok){
        ...
        if(response.status===403){csrf='';await token();}
      ...
      try{await token();}catch{csrf='';retry.hidden=false;}
    }catch{
      say('We could not confirm submission. Check your connection before trying again.','error');
    }finally{sending=false;submit.disabled=!csrf;submit.textContent=cfg.content.submit;retry.hidden=Boolean(csrf);form.removeAttribute('aria-busy');}
  });
}
